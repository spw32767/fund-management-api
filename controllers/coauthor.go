...
func UpdateCoauthor(c *gin.Context) {
    ...
    // Update order sequence if provided
    if req.OrderSequence > 0 {
        submissionUser.OrderSequence = req.OrderSequence
    }
...
func RemoveCoauthor(c *gin.Context) {
    ...
    for i, coauthor := range remainingCoauthors {
        coauthor.OrderSequence = i + 2 // Start from 2 (1 is main author)
        config.DB.Save(&coauthor)
    }
