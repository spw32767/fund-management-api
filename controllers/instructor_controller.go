...
// ─── Delete handlers — ทุกตัวผ่าน service เพื่อบันทึก audit log ──────────────

func DeleteInstructorTextbook(c *gin.Context) {
	id, editorID, ok := parseDeleteParams(c, "ID ตำราไม่ถูกต้อง")
	if !ok {
		return
	}
	svc := services.NewInstructorService(config.DB)
...
func DeleteInstructorIntellectualProperty(c *gin.Context) {
	id, editorID, ok := parseDeleteParams(c, "ID ทรัพย์สินทางปัญญาไม่ถูกต้อง")
	if !ok {
		return
	}
	svc := services.NewInstructorService(config.DB)
...
func DeleteInstructorResearchProject(c *gin.Context) {
	id, editorID, ok := parseDeleteParams(c, "ID โครงการวิจัยไม่ถูกต้อง")
	if !ok {
		return
	}
	svc := services.NewInstructorService(config.DB)
...
func DeleteInstructorExpertise(c *gin.Context) {
	id, editorID, ok := parseDeleteParams(c, "ID ความเชี่ยวชาญไม่ถูกต้อง")
	if !ok {
		return
	}
	svc := services.NewInstructorService(config.DB)
...
func DeleteInstructorEducation(c *gin.Context) {
	id, editorID, ok := parseDeleteParams(c, "ID ประวัติการศึกษาไม่ถูกต้อง")
	if !ok {
		return
	}
	svc := services.NewInstructorService(config.DB)
...
// ─── parseDeleteParams — ดึง :id จาก URL และ editorID จาก JWT ──────────────
// คืน (id, editorID, ok) — ถ้า ok=false แปลว่า response ส่งไปแล้ว ให้ return เลย
