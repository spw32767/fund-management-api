...
| # | เรื่อง | สถานะ |
|---|---|---|
| 1 | `custom` scope ประกอบ `extra_query` เป็น base ของ query | ✅ **แก้แล้ว** (`buildScopeQuery`) |
| 2 | comparison ใช้ `MAX(id)` แทน `MAX(captured_at)` (deterministic กันชนวินาที) | ✅ **แก้แล้ว** |
| 5 | กด "อัปเดตตัวเลข" ทีเดียว วนทุก scope พร้อมกัน → nesting คงเส้นคงวา | ✅ มีอยู่แล้ว (`counts/refresh`) |
| 3 | snapshot row สะสมเรื่อยๆ — housekeeping ลบของเก่า | ⏳ **ทำเมื่อ scale โต** (ตอนนี้ ~20 แถว) |
| 4 | faculty query ต่อ AU-ID ด้วย `OR` (ตอนนี้ 41 คน = 894 ตัวอักษร) — ถ้าเป็นหลักร้อยควร chunk + dedup ด้วย eid | ⏳ **ทำเมื่ออาจารย์หลักร้อย** |

**สรุป:** logic ถูกต้อง ปลอดภัยต่อการใช้งานจริง · ข้อ 1, 2 แก้แล้ว · ข้อ 3, 4 เป็นเรื่อง scale — จดไว้ทำเมื่อจำเป็น (ไม่แก้ปัญหาที่ยังไม่เกิด)
